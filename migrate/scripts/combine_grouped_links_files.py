...
# Read outgoing links
with gzip.open(OUTGOING_LINKS_FILE, 'rt', encoding='utf-8') as outgoing_file:
    for line in outgoing_file:
        source_page_id, target_page_ids = line.rstrip('\n').split('\t')
        LINKS[source_page_id]['outgoing'] = target_page_ids

# Read incoming links
with gzip.open(INCOMING_LINKS_FILE, 'rt', encoding='utf-8') as incoming_file:
    for line in incoming_file:
        target_page_id, source_page_ids = line.rstrip('\n').split('\t')
        LINKS[target_page_id]['incoming'] = source_page_ids

# For each page in the links dictionary, print out its incoming and outgoing links as well as their counts.
for page_id, links in LINKS.items():  # Updated iteritems() to items()
    ...
    columns = [
        page_id,
        str(outgoing_links_count),
        str(incoming_links_count),
        outgoing_links,
        incoming_links,
    ]
